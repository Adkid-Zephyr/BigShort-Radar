...
> 用户 lau 在 2026-05-18 下午做了最终同步,准备归还当前电脑。
> 让下一个接手的人(自己换电脑、或新助手)能够从零续上。
>
> GitHub 仓库 https://github.com/Adkid-Zephyr/BigShort-Radar
...
## 二、当前基线(HEAD e6f212e,iter 65 完成)

| 维度 | 现状 |
|---|---|
| 测试 | pytest 523 通过 / 0 失败 / 0 skip |
| 指标 | 24 条 / 8 维度(波动率 / 期权情绪 / 曲线 / 信用 / 流动性 / 跨市场 / 政策 / 中国) |
| 综合温度计 | 首页实时计算 latest 指标;维度内 max 触顶;切点 60;权重=曲线18/信用18/流动性13/波动率10/期权情绪8/跨市场13/政策10/中国10 |
| Web | 7 page 全活:`/` `/events` `/heatmap` `/timeline` `/hedge` `/calibration` `/indicator/<name>` |
| 数据源 | FRED + CBOE 官方 CSV/Page + 少量 Yahoo(残留/备用) |
| 数据库 | `data/radar.sqlite` 已包含最新主 DB;`historical_cache.sqlite` 已同步;`data/backtest_results/` 已同步 |
| 回测 | 三窗口 CSV 已出:2008 雷曼 / 2020 COVID / 2022 加息;`OLD_HISTORY_SOURCES.md` 已调研 1970s/1929 |
| 前端 | 信息驾驶舱方向;首页隐藏“今日风险简报”,保留右下角 AI 助手;环型温度计实时刷新 |
